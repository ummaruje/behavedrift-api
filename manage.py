...
async def reset_api_key(tenant_id: str):
    ...
    async with AsyncSessionLocal() as db:
        ...
        print("\n" + "="*50)
        print(f"API Key Reset Successfully!")
        print(f"Tenant ID: {tenant_id}")
...
def main():
    ...
    # create-tenant
    parser_create = subparsers.add_parser("create-tenant", help="Create a new tenant with an API key")
    parser_create.add_argument("name", type=str, help="Name of the tenant organization")
    
    # reset-key
    parser_reset = subparsers.add_parser("reset-key", help="Reset the API key for an existing tenant")
